...
2. Доступна директория `.data/workplace-init-bundle/` с шаблонами и скриптами для инициализации (этот промпт находится внутри бандла). Если директория отсутствует — запроси у пользователя путь к бандлу.
...
Stub спецификации создаётся из `templates/service-stub.md.template` с заменой `{{SERVICE_NAME}}` (человекочитаемое имя) и `{{SERVICE_CODE}}` (код из `projects_list`).
...
| `{{SERVICE_NAME}}` | Имя сервиса    | `My Service`     |
| `{{SERVICE_CODE}}` | Код сервиса    | `my-service`     |
